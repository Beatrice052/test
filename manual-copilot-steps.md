...
### 可选：比较 `/create-skill` 和 `skill-creator`

这个步骤只用于比较两种“创建 Skill 的方法”，不属于正式 A/C 主实验。不要把它的结果填进普通 `scorecard.md`。

1. 先完成上面的 `/create-skill` 创建流程，并保存创建出的 Skill 资产说明到 `scenarios/skill-api-contract-tests/results/skill-creator-comparison/run-01/copilot-created-skill-asset-notes.md`。
2. 在支持 `skill-creator` 的环境中新建会话。
3. 复制 `scenarios/skill-api-contract-tests/skill-creator-request.md` 的全部内容。
4. 发送并创建 `api-contract-test-design-skill-creator`。
5. 可用 `scenarios/skill-api-contract-tests/reference-asset/api-contract-test-design-skill-creator/` 对照，但不要把该参考资产默认安装到 `.github`。
6. 在新的 Copilot Chat 或支持该 Skill 的目标环境中，复制 `scenarios/skill-api-contract-tests/customized-input.md`，运行 `/api-contract-test-design`，保存原始输出到 `results/skill-creator-comparison/run-01/copilot-created-skill-output.md`。
7. 新建 Chat，复制 `scenarios/skill-api-contract-tests/customized-input-skill-creator.md`，运行 `/api-contract-test-design-skill-creator`，保存原始输出到 `results/skill-creator-comparison/run-01/skill-creator-created-skill-output.md`。
8. 用 `scenarios/skill-api-contract-tests/skill-creation-comparison-scorecard.md` 人工打分，并把结果写入 `results/skill-creator-comparison/run-01/scorecard.md`。

比较时只允许改变 Skill 创建方法和对应 Slash Command。API Contract 正文必须完全相同；每次运行都使用新 Chat。
