...
这个场景不需要真实数据库。输入只包含数据字典、样例数据和报表问题。

1. 新建默认 Chat，复制 `scenarios/prompt-nl2sql/novice-input.md`，保存为 A。
2. 新建 Agent Chat，复制 `scenarios/prompt-nl2sql/create-customization-request.md`，执行 `/create-prompt`。
3. 确认 Workspace Prompt `safe-nl2sql` 成功创建。
4. 新建 Chat，复制 `scenarios/prompt-nl2sql/customized-input.md`，保存为 C。
5. 填写 `scenarios/prompt-nl2sql/scorecard.md`。
