...
本场景模拟真实 NL2SQL 工作方式：项目 Workspace 里已经有数据库说明文件 `workspace-context/ecommerce_database.md`，用户不会重新粘贴完整数据库文档，但会在提问时粗略提到相关表和字段。测试重点是 Prompt File 是否能把“读取 Workspace 数据上下文、识别业务口径、避免多表重复聚合、生成可审核 SQL”的专家方法封装起来。

## Local Reference Only

- `workspace-context/ecommerce_database.md`：模拟 Workspace 中已经存在的数据库说明。
...
2. 打开 `scenarios/prompt-nl2sql/workspace-context/ecommerce_database.md`，或在 Copilot Chat 中把该文件添加为上下文。
3. A 组和 C 组必须使用同一个数据库上下文文件。
4. 不要把 `workspace-context/ecommerce_database.md` 的全文复制进 `novice-input.md` 或 `customized-input.md`。
5. 不要额外提供真实数据库、建表语句、样例数据或执行结果。

## Group A: Baseline

1. 打开新的默认 Copilot Chat。
2. 确认 `workspace-context/ecommerce_database.md` 已作为当前 Chat 可见的 Workspace/Chat context。
...
- A 和 C 使用同一个 `workspace-context/ecommerce_database.md`。
- A 和 C 的用户查询内容必须相同，唯一变量是是否使用 `/safe-nl2sql`。
- 模型、VS Code、Copilot 扩展版本和可见上下文必须相同。
- A 和 C 均使用新 Chat。
- 不根据输出结果临时改 Prompt 或数据库上下文。
- 如果 Prompt 资产、`novice-input.md`、`customized-input.md` 或数据库上下文发生修改，A/C 两组都需要重新运行。
